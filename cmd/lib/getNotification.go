...
func GetNotifications(w http.ResponseWriter, userUUID string, data map[string]interface{}) map[string]interface{} {
	...
	notificationsQuery := `
		SELECT ID, Reaction_ID, Post_ID, Comment_ID, CreatedAt, IsRead	FROM Notification WHERE User_UUID = ?ORDER BY CreatedAt DESC`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&notification.ID, &notification.ReactionID, &notification.PostID, &notification.CommentID, &createdAt, &notification.IsRead); err != nil {
